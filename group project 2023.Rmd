...
```
    ...
    #The project will consist of Justin and I coming together to use the SQL database to record the data obtained from the deer data 
    collected from this years capstoe. The data consist of qualitative and quantitative data such as deer ID numbers, harvest date, 
    year, county, area name, age, sex among many other categories in which data was accounted for. Our goal as a whole is to be able to 
    organize the obtained data in a way that is approchable and facilitates anaylsis of data once it comes to that. The data that has       been generated thus far is currently recorded through an Excel sheet, and is in the process of being transfered to the SQL database 
    as codes are generated and chosen fit for this project. 
 
    #This creates the Table and the add creates columns
    #The Varchar is the amount of character for the name of the column we did 50 so there is a confortarble amount of room incase the 
    name is changed.
...
ALTER TABLE CapstoneCWD;
 ...
 ADD CWD VARCHAR(50);
  ...
  VALUES (2318, "2023-02-13", "McIntosh", "NE", 6.5, "F", 110, "Good", "No),
    ...
    #the data within the columns was organized in a way to easily identify the deer in a speciic county within a specific date. The 
    way in which the data was inputed allows us as scientist to facilitate access and interpretation. The rows and columns are 
    mostly arranged from most to least importance for the purpose of this research. 
